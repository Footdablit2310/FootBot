    ...
    async def delete_map(self, interaction: discord.Interaction) -> None:
        ...
        options = [
            discord.SelectOption(
                label=m["name"], description=f"{m['difficulty']} ({m['points']} pts)"
            )
            for m in maps_data
        ]

        select: discord.ui.Select[Any] = discord.ui.Select(
            placeholder="Choose a map to delete", options=options
        )
        ...
        async def on_select(interaction: discord.Interaction):
            ...
            if not chosen_map:
                await interaction.response.send_message(
                    f"No map named **{mapname}** found.", ephemeral=True
                )
                return
            ...
            await interaction.response.edit_message(
                content=f"Map **{mapname}** has been deleted.", view=None
            )
        ...
        await interaction.response.send_message(
            "Select a map to delete:", view=view, ephemeral=True
        )
