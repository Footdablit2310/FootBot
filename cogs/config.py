...
class Config(commands.Cog):
    ...
    async def config(self, interaction: discord.Interaction, key: Optional[str] = None, value: Optional[str] = None) -> None:
        ...
        if key and value:
            ...
            await interaction.response.send_message(f"⚙️ Config {key} updated to {value}.", ephemeral=True)
            return
        await interaction.response.send_message(f"⚙️ Current config: {data['config']}", ephemeral=True)
